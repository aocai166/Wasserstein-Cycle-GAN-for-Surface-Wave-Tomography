...
class DataLoader():
    def __init__(self, file_train_path, test_disp_path, out_path,
        ...
        self.file_disp_path = self.file_train_path + 'disp_region_pos/'
...
#        out = lst[0].split(',')
    ...
    def load_data(self, fulldata=True, tdtype='npy'):
        ...
        Ldisp_data = np.zeros((self.ntrain, self.disp_sample, 4))
        ...
        for file in self.file_name_train:
            ...
            Ldisp_data[itrain, :, :] = tmp_disp[:, 1:5]
            tmp_vs = np.loadtxt(self.file_vs_path+file)
            Lvs_data[itrain, :, 0] = tmp_vs[:, 1]
            itrain += 1

        Udisp_data = np.zeros((self.ntest, self.disp_sample, 4))

        if tdtype=='npy':
            tmp_vol = np.load(self.file_train_path+self.tdname)
            Udisp_data[:, :, 0:2] = tmp_vol[:, :, 3:5]
            Udisp_data[:, :, 2] = tmp_vol[:, :, 1]
            Udisp_data[:, :, 3] = -1.0 * tmp_vol[:, :, 0]
            ...
            for file in self.file_name_test:
                ...
                Udisp_data[itest, :, :] = tmp_disp[:, 1:5]
                itest += 1

        train_disp_max = np.max(Ldisp_data[:, :, 0:2])
        test_disp_max = np.max(Udisp_data[:, :, 0:2])
        self.disp_max = max(train_disp_max, test_disp_max)
        train_lat_max = np.max(Ldisp_data[:, :, 2])
        test_lat_max = np.max(Udisp_data[:, :, 2])
        self.lat_max = max(train_lat_max, test_lat_max)
        train_lon_max = np.max(Ldisp_data[:, :, 3])
        test_lon_max = np.max(Udisp_data[:, :, 3])
        self.lon_max = max(train_lon_max, test_lon_max)

        train_disp_min = np.min(Ldisp_data[:, :, 0:2])
        test_disp_min = np.min(Udisp_data[:, :, 0:2])
        self.disp_min = min(train_disp_min, test_disp_min)
        train_lat_min = np.min(Ldisp_data[:, :, 2])
        test_lat_min = np.min(Udisp_data[:, :, 2])
        self.lat_min = min(train_lat_min, test_lat_min)
        train_lon_min = np.min(Ldisp_data[:, :, 3])
        test_lon_min = np.min(Udisp_data[:, :, 3])
        self.lon_min = min(train_lon_min, test_lon_min)
        ...
        if fulldata:
            return Ldisp_data, Lvs_data, Udisp_data, self.disp_max, self.lat_max, self.lon_max, self.vs_max, \
                   self.disp_min, self.lat_min, self.lon_min, self.vs_min, self.file_name_train, self.file_name_test
        else:
            return self.disp_max, self.lat_max, self.lon_max, self.vs_max, self.disp_min, self.lat_min, \
                   self.lon_min, self.vs_min, self.file_name_train, self.file_name_test
    ...
    def load_batch_data(self, ibatch=0):
        ...
        Ldisp_batch = np.zeros((self.Lsize, self.disp_sample, 4))
        Lvs_batch = np.zeros((self.Lsize, self.vs_sample, 1))

        itrain = 0
        for file in Lfile:
            tmp_disp = np.loadtxt(self.file_disp_path + file)
            Ldisp_batch[itrain, :, :] = tmp_disp[:, 1:5]
            tmp_vs = np.loadtxt(self.file_vs_path + file)
        ...
        Udisp_batch = np.zeros((self.Usize, self.disp_sample, 4))

        itest = 0
        for file in Ufile:
            tmp_disp = np.loadtxt(self.test_disp_path + file)
            Udisp_batch[itest, :, :] = tmp_disp[:, 1:5]
            itest += 1
    ...
    def load_test_data(self, nselect=100, sample=True, seed=None, randpick=False, getpick=False, tdtype='npy'):
        ...
        if sample:
            ...
            disp_test = np.zeros((nselect, self.disp_sample, 4))
            ...
            if tdtype == 'npy':
                ...
                disp_test = np.zeros((self.ntest, self.disp_sample, 4))
    ...
    def load_label_data(self, nselect=100, sample=True, seed=None, randpick=False, getpick=False):
        ...
        if sample:
            ...
            disp_select = np.zeros((nselect, self.disp_sample, 4))
            ...
            for iselect in select:
                ...
                disp_select[ipick, :, :] = tmp_disp[:, 1:5]
            ...
            disp_select = np.zeros((self.ntrain, self.disp_sample, 4))
