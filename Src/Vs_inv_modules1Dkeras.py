...
def d_layer(layer_input, filters, f_size=3, stride=2, name="d_layer"):
    ...
    with tf.variable_scope(name):
        ...
        #d = tfa.layers.GroupNormalization(groups=int(filters/2))(d)
        return d
...
def generator_dcnn_P2G(input_shape=vs_dim_default, gf=gf_default, channels=2, name="generatorP2G"):
    with tf.variable_scope(name):
        img = tf.keras.layers.Input(shape=input_shape)
