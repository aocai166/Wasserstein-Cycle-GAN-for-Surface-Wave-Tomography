...
# -*- coding: utf-8 -*-
"""
Original version on 05/26/2020
Updated version 02/18/2022

@author: aocai (aocai166@gmail.com) Rice University
"""
# Cycle GAN with Wasserstein metric and gradient penalty
...
from Vs_inv_pos_modules1Dkeras import *
from Vs_inv_pos_data_loader import DataLoader
...
class CycleGAN():
    def __init__(self, sess):
        ...
        # Path to the folder of labeled data (e.g., disp_region_pos/)
        ...
        self.test_disp_path = 'D:/PycharmProjects/GPUpy/Qiu_data/CVMHTrainingDataset/linear_test_pos_new/'
        # Path to the directory where you want to store the results
        self.out_path = 'D:/PycharmProjects/GPUpy/Qiu_data/CVMHTrainingDataset/Final_results/output_region_pos/'
        self.test_dtype = 'txt'                                            # type of test data, vary between txt and npy
        self.save_freq = 10
    ...
    def train(self, epochs, batch_size=10, sample_interval=1):
        ...
        Ldisp_data_orig, Lvs_data_orig, Udisp_data_orig, self.disp_max, self.lat_max, self.lon_max, self.vs_max, \
        self.disp_min, self.lat_min, self.lon_min, self.vs_min,self.file_name_train, self.file_name_test \
            = self.data_loader.load_data(fulldata=True, tdtype=self.test_dtype)
        print(self.disp_max, self.lat_max, self.lon_max, self.disp_min, self.lat_min, self.lon_min,
              self.vs_max, self.vs_min)

        if self.lat_lon_norm:
            latlon_max = max(self.lat_max, self.lon_max)
            latlon_min = min(self.lat_min, self.lon_min)
            self.lat_max = latlon_max
            self.lon_max = latlon_max
            self.lat_min = latlon_min
            self.lon_min = latlon_min

        # Normalize the data
        if self.data_normalize:
            if self.balance_norm:
                Ldisp_data_orig[:,:,0:2] = balance_normalization(Ldisp_data_orig[:,:,0:2], self.disp_max, self.disp_min)
                Ldisp_data_orig[:,:,2] = balance_normalization(Ldisp_data_orig[:,:,2], self.lat_max, self.lat_min)
                Ldisp_data_orig[:,:,3] = balance_normalization(Ldisp_data_orig[:,:,3], self.lon_max, self.lon_min)
                Lvs_data_orig = balance_normalization(Lvs_data_orig, self.vs_max, self.vs_min)
                Udisp_data_orig[:,:,0:2] = balance_normalization(Udisp_data_orig[:,:,0:2], self.disp_max, self.disp_min)
                Udisp_data_orig[:,:,2] = balance_normalization(Udisp_data_orig[:,:,2], self.lat_max, self.lat_min)
                Udisp_data_orig[:,:,3] = balance_normalization(Udisp_data_orig[:,:,3], self.lon_max, self.lon_min)
            else:
        ...
        for epoch in range(epochs):
            ...
            for ibatch in range(nbatch):
                ...
                print(("Epoch: [%2d/%2d] Lbatch: [%4d/%4d] Ubatch: [%4d/%4d] Loss:%4.6f time: %4.4f" % (
                        epoch, epochs, Lkbatch, self.Lbatch_rate, Ukbatch, self.Ubatch_rate,
                        loss_val, time.time() - start_time)))
    ...
    def predict_vol(self, direction='G2P'):
        ...
        init_op = tf.global_variables_initializer()
        self.sess.run(init_op)
        _, _, _, self.disp_max, self.lat_max, self.lon_max, self.vs_max, self.disp_min, self.lat_min, self.lon_min, \
        self.vs_min, self.file_name_train, self.file_name_test = self.data_loader.load_data(fulldata=True,
                                                                                            tdtype=self.test_dtype)
        print(self.disp_max, self.lat_max, self.lon_max, self.vs_max, self.disp_min, self.lat_min,
              self.lon_min, self.vs_min)
        ...
        disp_full = np.zeros((test_full.shape[0], test_full.shape[1], 4))
        disp_full[:,:,0:2] = test_full[:, :, 3:5]
        disp_full[:,:,2] = test_full[:, :, 1]
        disp_full[:,:,3] = -1.0 * test_full[:, :, 0]

        if self.data_normalize:
            if self.balance_norm:
                disp_full[:,:,0:2] = balance_normalization(disp_full[:,:,0:2], self.disp_max, self.disp_min)
                disp_full[:,:,2] = balance_normalization(disp_full[:,:,2], self.lat_max, self.lat_min)
                disp_full[:,:,3] = balance_normalization(disp_full[:,:,3], self.lon_max, self.lon_min)
            else:
                disp_full = disp_full / self.disp_max

        fake_vs = np.zeros((disp_full.shape[0], self.vs_dim, self.vs_channels))
        ...
        for ibatch in range(nbatch + 1):
            ...
            reconstr_disp[left:right, :, 3:5] = reconstr_disp_batch[:, :, 0:2]
        ...
        np.save(self.out_path + 'predict/' + 'Predict_vs_pos_v1.npy', fake_vs)
        np.save(self.out_path + 'predict/' + 'Reconstr_disp_pos_v1.npy', reconstr_disp)
    ...
    def test(self, direction='G2P', drawline=False):
        ...
        if direction == 'G2P':
            titles = ['Labeled Vph', 'Labeled Vgp', 'Translated Vs', 'Labeled Vs', 'Reconstructed Vph',
                      'Reconstructed Vgp']
            disp_select, vs_select = self.data_loader.load_label_data(nselect=100, sample=True)

            if self.data_normalize:
                if self.balance_norm:
                    disp_select[:,:,0:2] = balance_normalization(disp_select[:,:,0:2], self.disp_max, self.disp_min)
                    disp_select[:, :, 2] = balance_normalization(disp_select[:, :, 2], self.lat_max, self.lat_min)
                    disp_select[:, :, 3] = balance_normalization(disp_select[:, :, 3], self.lon_max, self.lon_min)
                else:
            ...
            if self.data_normalize:
                if self.balance_norm:
                    disp_select[:,:,0:2] = balance_norm_reverse(disp_select[:,:,0:2], self.disp_max, self.disp_min)
                    disp_select[:,:,2] = balance_norm_reverse(disp_select[:,:,2], self.lat_max, self.lat_min)
                    disp_select[:,:,3] = balance_norm_reverse(disp_select[:,:,3], self.lon_max, self.lon_min)
                    fake_vs = balance_norm_reverse(fake_vs, self.vs_max, self.vs_min)
                    reconstr_disp[:,:,0:2] = balance_norm_reverse(reconstr_disp[:,:,0:2], self.disp_max, self.disp_min)
                    reconstr_disp[:,:,2] = balance_norm_reverse(reconstr_disp[:,:,2], self.lat_max, self.lat_min)
                    reconstr_disp[:,:,3] = balance_norm_reverse(reconstr_disp[:,:,3], self.lon_max, self.lon_min)
                else:
                    disp_select = disp_select * self.disp_max
                    fake_vs = fake_vs * self.vs_max
            ...
            if self.data_normalize:
                if self.balance_norm:
                    ...
                    fake_disp[:,:,0:2] = balance_norm_reverse(fake_disp[:,:,0:2], self.disp_max, self.disp_min)
                    fake_disp[:,:,2] = balance_norm_reverse(fake_disp[:,:,2], self.lat_max, self.lat_min)
                    fake_disp[:,:,3] = balance_norm_reverse(fake_disp[:,:,3], self.lon_max, self.lon_min)
    ...
    def rms_misfit(self, mode='Test'):
        ...
        Ldisp_data_orig, Lvs_data_orig, _, self.disp_max, self.lat_max, self.lon_max, self.vs_max, \
        self.disp_min, self.lat_min, self.lon_min, self.vs_min, _, _ \
            = self.data_loader.load_data(fulldata=True, tdtype=self.test_dtype)
        print(self.disp_max, self.lat_max, self.lon_max, self.disp_min, self.lat_min, self.lon_min,
              self.vs_max, self.vs_min)
    ...
    def sample_vsprofile(self, epoch):
        ...
        for i in range(r):
            ...
            if self.data_normalize:
                if self.balance_norm:
                    disp_test[:,:,0:2] = balance_normalization(disp_test[:,:,0:2], self.disp_max, self.disp_min)
                    disp_test[:,:,2] = balance_normalization(disp_test[:,:,2], self.lat_max, self.lat_min)
                    disp_test[:,:,3] = balance_normalization(disp_test[:,:,3], self.lon_max, self.lon_min)
                else:
            ...
            if self.data_normalize:
                if self.balance_norm:
                    disp_test[:,:,0:2] = balance_norm_reverse(disp_test[:,:,0:2], self.disp_max, self.disp_min)
                    disp_test[:,:,2] = balance_norm_reverse(disp_test[:,:,2], self.lat_max, self.lat_min)
                    disp_test[:,:,3] = balance_norm_reverse(disp_test[:,:,3], self.lon_max, self.lon_min)
                    fake_vs = balance_norm_reverse(fake_vs, self.vs_max, self.vs_min)
                    reconstr_disp[:,:,0:2] = balance_norm_reverse(reconstr_disp[:,:,0:2], self.disp_max, self.disp_min)
                    reconstr_disp[:,:,2] = balance_norm_reverse(reconstr_disp[:,:,2], self.lat_max, self.lat_min)
                    reconstr_disp[:,:,3] = balance_norm_reverse(reconstr_disp[:,:,3], self.lon_max, self.lon_min)
    ...
    def line_vsprofile(self, epoch):
        ...
        if self.data_normalize:
            if self.balance_norm:
                disp_train[:,:,0:2] = balance_normalization(disp_train[:,:,0:2], self.disp_max, self.disp_min)
                disp_train[:,:,2] = balance_normalization(disp_train[:,:,2], self.lat_max, self.lat_min)
                disp_train[:,:,3] = balance_normalization(disp_train[:,:,3], self.lon_max, self.lon_min)
                disp_test[:,:,0:2] = balance_normalization(disp_test[:,:,0:2], self.disp_max, self.disp_min)
                disp_test[:,:,2] = balance_normalization(disp_test[:,:,2], self.lat_max, self.lat_min)
                disp_test[:,:,3] = balance_normalization(disp_test[:,:,3], self.lon_max, self.lon_min)
            else:
        ...
        if self.data_normalize:
            if self.balance_norm:
                disp_train[:, :, 0:2] = balance_norm_reverse(disp_train[:, :, 0:2], self.disp_max, self.disp_min)
                disp_train[:, :, 2] = balance_norm_reverse(disp_train[:, :, 2], self.lat_max, self.lat_min)
                disp_train[:, :, 3] = balance_norm_reverse(disp_train[:, :, 3], self.lon_max, self.lon_min)
                fake_vs_train = balance_norm_reverse(fake_vs_train, self.vs_max, self.vs_min)
                reconstr_disp_train[:, :, 0:2] = balance_norm_reverse(reconstr_disp_train[:, :, 0:2], self.disp_max, self.disp_min)
                reconstr_disp_train[:, :, 2] = balance_norm_reverse(reconstr_disp_train[:, :, 2], self.lat_max, self.lat_min)
                reconstr_disp_train[:, :, 3] = balance_norm_reverse(reconstr_disp_train[:, :, 3], self.lon_max, self.lon_min)
                disp_test[:, :, 0:2] = balance_norm_reverse(disp_test[:, :, 0:2], self.disp_max, self.disp_min)
                disp_test[:, :, 2] = balance_norm_reverse(disp_test[:, :, 2], self.lat_max, self.lat_min)
                disp_test[:, :, 3] = balance_norm_reverse(disp_test[:, :, 3], self.lon_max, self.lon_min)
                fake_vs_test = balance_norm_reverse(fake_vs_test, self.vs_max, self.vs_min)
                reconstr_disp_test[:, :, 0:2] = balance_norm_reverse(reconstr_disp_test[:, :, 0:2], self.disp_max, self.disp_min)
                reconstr_disp_test[:, :, 2] = balance_norm_reverse(reconstr_disp_test[:, :, 2], self.lat_max, self.lat_min)
                reconstr_disp_test[:, :, 3] = balance_norm_reverse(reconstr_disp_test[:, :, 3], self.lon_max, self.lon_min)
            else:
                disp_train = disp_train * self.disp_max
    ...
    def model_uncertainty_est(self, type='PerturbNets', noisetype='Gaussian',
        ...
        self.sess.run(init_op)
        _, _, _, self.disp_max, self.lat_max, self.lon_max, self.vs_max, self.disp_min, self.lat_min, self.lon_min, \
        self.vs_min, self.file_name_train, self.file_name_test = self.data_loader.load_data(fulldata=True, tdtype=self.test_dtype)
        print(self.disp_max, self.lat_max, self.lon_max, self.vs_max, self.disp_min, self.lat_min,
              self.lon_min, self.vs_min)
        ...
        if type == 'PerturbNets':
            ...
            vs_name = 'Predict_vs_epoch_%d_%d.npy' % (epoch_start, epoch_curr)
            ...
            test_full = self.data_loader.load_full_test_data()
            disp_orig = np.zeros((test_full.shape[0], test_full.shape[1], 4))
            disp_orig[:, :, 0:2] = test_full[:, :, 3:5]
            disp_orig[:, :, 2] = test_full[:, :, 1]
            disp_orig[:, :, 3] = -1.0 * test_full[:, :, 0]
            ...
            vs_name = 'Predict_vs_perturbs_%d.npy' % (nperturb)
            np.save(output_dir + vs_name, fake_vs_full)
...
if __name__ == "__main__":
    ...
    mode = 'Mod_uncer'
    if mode == 'Train':
        gan.train(epochs=101, batch_size=5, sample_interval=10)
    elif mode == 'Test':
        gan.test(direction='G2P', drawline=False)
        loss = gan.rms_misfit(mode=mode)
        print(loss)
    elif mode == 'Predict':
        gan.predict_vol(direction='G2P')
    elif mode == 'Mod_uncer':
        # gan.model_uncertainty_est(type='PerturbNets', epoch_start=0, epoch_jump=10, njumps=11)
        gan.model_uncertainty_est(type='PerturbData', nperturb=100, noisetype='Uniform')
    else:
