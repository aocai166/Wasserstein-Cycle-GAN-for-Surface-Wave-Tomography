...
"""
# Cycle-GAN / Least-squares GAN using tensorflow for deriving Vs model from dispersion curve
...
class CycleGAN():
    def __init__(self, sess):
        ...
        self.cycle_loss = True       # Cycle-consistent loss: For a complete GAN structure, it should always be true.
        self.est_loss = True         # Estimation loss : For a complete GAN structure, it should always be true.
        self.cycle_mse = True
        self.est_mse = True
        self.n_generate = 3

        # Path to the folder of labeled data (e.g., disp_region/)
        self.file_train_path = 'D:/PycharmProjects/GPUpy/Qiu_data/CVMHTrainingDataset/'
        ...
        self.out_path = 'D:/PycharmProjects/GPUpy/Qiu_data/CVMHTrainingDataset/LS_output/'
        ...
        self.lambda_est = 0.2 * self.lambda_cycle
    ...
    def _build_model(self):
        ...
        self.dp_loss_fake = self.criterionGAN(self.DP_fake_sampleL, tf.zeros_like(self.DP_fake_sampleL)) \
                          + self.criterionGAN(self.DP_fake_sampleU, tf.zeros_like(self.DP_fake_sampleU))
        self.dp_loss = (self.dp_loss_real + self.dp_loss_fake) / 2
        ...
        self.dg_loss_fake = self.criterionGAN(self.DG_fake_sample, tf.zeros_like(self.DG_fake_sample))
        self.dg_loss = (self.dg_loss_real + self.dg_loss_fake) / 2
    ...
    def train(self, epochs, batch_size=10, sample_interval=1):
        ...
        for epoch in range(epochs):
            ...
            for ibatch in range(nbatch):
                ...
                counter += 1
                print(("Epoch: [%2d/%2d] Lbatch: [%4d/%4d] Ubatch: [%4d/%4d] Loss: %2.6f time: %4.4f" % (
    ...
    def predict_vol(self, direction='G2P'):
        ...
        np.save(self.out_path + 'predict/' + 'Cyc_Predict_vs.npy', fake_vs)
        np.save(self.out_path + 'predict/' + 'Cyc_Reconstr_disp.npy', reconstr_disp)
    ...
    def test(self, direction='G2P', drawline=False):
        ...
        if direction == 'G2P':
            ...
            for j in range(c):
                ...
                axs[j].axis('off')
            fig.savefig(self.out_path + "predict/" + "V2Vs_cycinv_test")
            plt.close()
            ...
            for j in range(c):
                ...
                axs[j].set_title(titles[j])
                axs[j].axis('off')
            fig.savefig(self.out_path + "predict/" + "Vs2V_cycinv_test")
            plt.close()
    ...
    def sample_vsprofile(self, epoch):
        ...
        for i in range(r):
            ...
            for j in range(c):
                ...
                axs[j].set_title(titles[j])
                axs[j].axis('off')
            fig.savefig(self.out_path + "Vsmodel_cycinv_e%d" % epoch)
            plt.close()
    ...
    def label_vsprofile(self, epoch):
        ...
        for j in range(c):
            ...
            axs[j].set_title(titles[j])
            axs[j].axis('off')
        fig.savefig(self.out_path + "Labeled_cycinv_e%d" % epoch)
        plt.close()
    ...
    def line_vsprofile(self, epoch):
        ...
        for j in range(c):
            if j < 2:
                ...
                data[:, j + 2] = pred_trace
            axs[j].invert_yaxis()
        fig.savefig(self.out_path + "predict/" + "Sample_cycinv2d_e%d" % epoch)
        plt.close()
        np.savetxt(self.out_path+"predict/"+"CycGAN_train_%d_%d_test_%d_%d.txt" % (pick_train[0], pick_train[1], pick_test[0], pick_test[1]), data, '%3.4f')
...
if __name__ == "__main__":
    ...
    mode = 'Test'
    if mode == 'Train':
        gan.train(epochs=501, batch_size=5, sample_interval=20)
    elif mode == 'Test':
